...
def coverage(mu, var, y, alpha=0.9):
    sigma = np.sqrt(np.maximum(var, 1e-8))
    lo = mu + norm.ppf((1-alpha)/2.0)*sigma
    hi = mu + norm.ppf(1 - (1-alpha)/2.0)*sigma
    covered = (y >= lo) & (y <= hi)
    return float(np.mean(covered)), int(np.sum(covered)), int(covered.size)

def ece_regression(mu, var, y, bins=10, alpha=0.9):
    sigma = np.sqrt(np.maximum(var, 1e-8))
    width = 2*norm.ppf(1 - (1-alpha)/2.0)*sigma
    qs = np.quantile(width, np.linspace(0,1,bins+1))
    ece = 0.0; n = 0
    for i in range(bins):
        if i < bins-1:
            m = (width >= qs[i]) & (width < qs[i+1])
        else:
            m = (width >= qs[i]) & (width <= qs[i+1])
        if m.sum()==0:
            continue
        cov,_,_ = coverage(mu[m], var[m], y[m], alpha)
        ece += abs(cov - alpha) * m.sum()
        n += m.sum()
    return float(ece/max(n,1))

def variance_error_correlation(mu, var, y):
    err = np.abs(mu - y).reshape(-1)
    v = var.reshape(-1)
    if err.size < 2:
        return 0.0
...
def aurc(mu, var, y, steps=20):
    ...
    for k in range(1, steps+1):
        keep = order[k*N//steps:]
        risks.append(np.mean(err[keep]))
        coverages.append(keep.size / N)
